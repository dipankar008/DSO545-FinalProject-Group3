...
ui <- shinyUI(
    ...
    tabPanel("Region Based Attribute Analysis",
             ...
             fluidRow(
               column(3, offset = 3, selectizeInput(inputId = "region", 
                                                    label = "Select region" , 
               ...
               column(3, selectizeInput(inputId = "year", 
                                        label = "Select year" , 
                                        choices = c("2015", 
                                                    "2016",
                                                    "2017"), 
                                        selected = NULL, 
                                        multiple = FALSE )),
               column(3, helpText("Hover over the Circle Marker to get the Country Details"))
               
               ),
             
             fluidRow(leafletOutput(outputId = "int_world_map",height = 700))),
    
    tabPanel("Happiness Score vs Different Attributes ", 
             sidebarLayout(
               sidebarPanel(width = 3, selectInput(inputId = "DualAxisIn",
                         label = "Select which parameter you want to compare with Happiness Score", 
                         choices = c("Trust",
                                     "Family",
                                     "Economy",
                                     "Freedom",
                         ...
                         selected = NA)),
               mainPanel(
                 plotOutput(outputId = "DualAxisOut", height = 600)
               ))
             )
 ...
 output$XY <- renderPlot({
  ...
  output$graph <- renderPlot({
    ...
    WHR %>%
      ...
      theme_bw() + ylim(3,8)  +theme(legend.position = "bottom")
  ...
  output$map <- renderLeaflet({
    ...
    target <- subset(worldSpatial, country %in% cnt)
  ...
  output$int_world_map <- renderLeaflet({
    ...
    source("world_interactive_map_func.r")
    plot_int_world_map(input$region,input$year)
    
    
  })
  
  output$DualAxisOut <- renderPlot({
    source("DualAxisScatterPlot.R")
    Print(input$DualAxisIn)
  })
